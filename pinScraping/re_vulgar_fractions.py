import re
#This code replaces any vulgar fractions in a string and replaces them with decimals
#This code accounts for mixed numbers but only accounts for one vulgar fraction per line
...
def get_regex(input):
    ...
    for sign, d, f in re.findall(rx, input):
...
def check_vulgar(input):
    ...
    input = re.sub(rx, get_regex(input), input)

  
 
    return input

check_vulgar('3½ cup Dijon mustard')
